...
RAWDoc.h, RAWDoc.cpp - 문서
    이 파일에는 CRAWDoc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 CRAWDoc::Serialize를 통해 파일 저장 및 로드를 구현합니다.
    문서에는 다음과 같은 문자열이 있습니다.
        파일 확장명:      raw
        파일 형식 ID:        RawImage.Document
        주 프레임 캡션:  RawImage
        Doc 형식 이름:       RawImage
        필터 이름:         Raw Image Files (*.raw)
        파일의 새 약식 이름: RawImage
        파일 형식의 긴 이름: RawImage.Document

RAWView.h, RAWView.cpp - 문서 뷰
    이 파일에는 CRAWView 클래스가 포함됩니다.
    CRAWView 개체는 CRAWDoc 개체를 보는 데 사용됩니다.

res\ImageTransformerDoc.ico
    아이콘 파일이며, CRAWDoc 클래스 MDII 자식 창의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 ImageTransformer.rc에 의해 포함됩니다.
